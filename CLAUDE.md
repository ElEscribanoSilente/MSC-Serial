...
- **2026-07-06 — Round-trip de dataclass `frozen` roto (BAJO):** el encoder serializaba los fields pero el decoder los asignaba con `setattr`, que una instancia frozen prohíbe (`FrozenInstanceError` → `MSCDecodeError`). **Fix:** `object.__setattr__` en la rama dataclass, como el `__init__` generado. **Detectarlo antes:** todo tipo soportado necesita test de round-trip incluidas sus variantes (frozen, slots, con `__getstate__`). Ancla: `test_frozen_dataclass_roundtrip`.

## Hallazgos de auditoría (peritaje 2026-07-06) — todos resueltos

Los cuatro hallazgos (1 crítico, 2 medios, 1 bajo) están corregidos, cada uno con test ancla. Ver post-mortems arriba.
